...
# from config import *
# import boston_housing.data_plots
import pickle
...
def process_raw_boston():
    with open(BOSTON_RAW_DATASET_PATH, 'r') as file:
    ...
    data = []
    temp_row = []
    # Process the lines to combine split rows
    for i, line in enumerate(lines):
        # Clean up the line and split into values
        values = line.split()
        if i % 2 == 0:  # Odd-indexed lines (Python uses 0-based indexing)
            temp_row = [float(x) for x in values]
        else:  # Even-indexed lines
            temp_row.extend(float(x) for x in values)  # Append the rest of the columns
            data.append(temp_row)  # Save the complete row
    df = pd.DataFrame(data, columns=columns)
    df.to_pickle(BOSTON_DATASET_PATH)
    print("Data has been processed and saved as pickle file.")

def load_boston_data():
    # Define the path to the pickle file
    pickle_file_path = BOSTON_DATASET_PATH
